...
| 이슈 | 현상 | 확인 및 대응 |
| --- | --- | --- |
| 로그인 세션 끊김 및<br>재로그인 페이지 전환 | 로그인 후 LXP 메인 페이지 대신<br>`accounts/signin/history`로 이동해<br>후속 테스트가 실패할 수 있음 | 최종 URL과 인증 응답 상태를 확인하고,<br>재로그인 페이지면 기록 삭제 후 로그인 페이지로 돌아가<br>정보를 다시 입력해 메인 페이지 표시 여부를 확인함 |
