...
# 현재 test_case02.py가 있는 Load_Test 폴더 기준 CSV 경로
    ...
    try:
        ...
        if login_res.status_code == 200:
            ...
            if course_res.status_code == 200:
                ...
                print(
                    f"[{user_index + 1:02d}번 유저] "
                    f"🟡 로그인 성공했으나 후속 API 실패 "
                    f"(상태코드: {course_res.status_code})"
                )
...
def test_token_extraction_with_pytest(user_count):
    ...
    if not accounts:
        pytest.fail(
            "🚨 CSV 파일에서 계정을 하나도 읽지 못했습니다."
        )

    if len(accounts) < user_count:
        pytest.skip(
            f"🚨 계정 수가 부족하여 "
            f"{user_count}명 테스트를 생략합니다."
        )

    print(
        f"\n=== {user_count}명 토큰 동적 추출 및 "
        f"후속 API 연동 부하 테스트 시작 ==="
    )
    ...
    # 지표 산출
    error_rate = (
        fail_count / user_count
    ) * 100

    print(
        f"\n📊 [결과 요약 - "
        f"{user_count}명 토큰 연동 부하]"
    )
