...
# 현재 test_case03~07.py가 있는 Load_Test 폴더 기준 CSV 경로
...
def execute_sequential_flow(my_account, user_index):
    ...
    print(
        f"[{user_index + 1:02d}번 유저] "
        f"🚀 전체 트랜잭션 플로우 시작 ({my_id})"
    )
    ...
    try:
        ...
        if res_course.status_code == 200:
            print(
                f"  └ [1단계 통과] "
                f"과목 정보 로딩 성공 ({lat_course}ms)"
            )
        else:
            print(
                f"  └ [1단계 실패] "
                f"과목 로딩 상태코드: "
                f"{res_course.status_code}"
            )
            return False
        ...
        if res_enter.status_code in [200, 201]:
            print(
                f"  └ [2단계 통과] "
                f"시험 입장 성공 ({lat_enter}ms)"
            )
        else:
            print(
                f"  └ [2단계 실패] "
                f"시험 입장 상태코드: "
                f"{res_enter.status_code}"
            )
            return False

        # [LOAD-03 / Step 3] 동시 시험 답안 제출
        t3 = time.time()

        res_stop = session.post(
            TEST_STOP_URL,
            json={"course_id": 45},
            timeout=5,
        )
        ...
        if res_stop.status_code == 200:
            print(
                f"  └ [3단계 통과] "
                f"답안 제출 성공 ({lat_stop}ms)"
            )
        else:
            print(
                f"  └ [3단계 실패] "
                f"답안 제출 상태코드: "
                f"{res_stop.status_code}"
            )
            return False
...
def test_load_sequential_flow(user_count):
    ...
    print(
        f"\n=== 가상 유저 {user_count}명 동시 투입: "
        f"1~4단계 연쇄 트랜잭션 테스트 시작 ==="
    )
