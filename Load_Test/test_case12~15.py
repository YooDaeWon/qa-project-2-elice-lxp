...
TEST_RESET_URL = (
    "https://dev-qatrack-api.dev.elicer.io/org/{org}/lecture/test/reset/"
)


# 현재 test_case12~15.py가 있는 Load_Test 폴더 기준 CSV 경로
...
def execute_safety_controlled_flow(my_account, user_index):
    ...
    for loop in range(1, 4):
        try:
            ...
            # 모든 개별 요청마다 Latency 5초 초과 및
            # 500 에러 체크용 내부 함수
            def send_request_with_safety_check(
                method,
                url,
                **kwargs,
            ):
                ...
                # [통제 항목 1]
                # 500 Internal Server Error 발생 시 즉시 중단
            ...
            if res_login.status_code != 200:
                return (
                    False,
                    f"로그인 실패 "
                    f"(상태코드: {res_login.status_code})",
                )

            # 2. 과목 로딩
            res_course = send_request_with_safety_check(
                "GET",
                COURSE_URL,
                timeout=6,
            )
            ...
            if res_reset.status_code != 200:
                return (
                    False,
                    f"재응시 초기화 실패 "
                    f"(상태코드: {res_reset.status_code})",
                )

            completed_loops += 1

            # [통제 항목 4]
            # Sampler 간 3초 ~ 5초 임의 지연 적용
            if loop < 3:
...
def test_safety_controls_with_pytest(target_users):
    ...
    print(
        f"\n🛡️ [안전성 통제 검증] "
        f"{target_users}명 동시 부하 테스트 시작 "
        f"(Safety Controls 적용)"
    )
    ...
    with concurrent.futures.ThreadPoolExecutor(
        ...
        futures = [
            executor.submit(
                execute_safety_controlled_flow,
                accounts[i],
                i,
            )
    ...
    assert error_rate < 1.0, (
        ...
        f"허용 에러율(1%)을 초과했거나 "
        f"안전성 통제(Kill Switch 등)에 의해 "
        f"테스트가 중단되었습니다. "
        f"사유: {failure_reasons}"
    )