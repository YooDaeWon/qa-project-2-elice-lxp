...
# 현재 이 파이썬 파일이 있는 Load_Test 폴더 기준으로 CSV 경로 설정
...
def test_login_load_with_pytest(user_count):
    ...
    assert error_rate < 1.0, (
        f"로그인 부하 테스트 실패: "
        f"{user_count}명 환경에서 에러율이 "
        f"1%를 초과했습니다 ({error_rate}%)"
    )