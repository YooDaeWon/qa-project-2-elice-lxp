...
class ExamPreparePage:
    ...
    TIMEOUT_URL = re.compile(r"/test(?:/onboard/info)?/?(?:\?.*)?$")
    ...
    def verify_timeout_loaded(self):
        """제한 시간 시험 준비 페이지 확인"""
        expect(self.page).to_have_url(self.TIMEOUT_URL, timeout=60_000)
        expect(self.agreement_checkbox).to_be_visible()
