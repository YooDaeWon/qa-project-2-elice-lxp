...
def execute_single_user_flow(my_account):
    ...
    try:
        # 1. 로그인
        if session.post(LOGIN_URL, json=payload, timeout=5).status_code != 200:
            return False, 0
        # 2. 과목 로딩
        if session.get(COURSE_URL, timeout=5).status_code != 200:
            return False, 0
        # 3. 시험 입장
        if session.post(
            TEST_ENTER_URL, json={"course_id": 45}, timeout=5
        ).status_code not in [200, 201]:
            return False, 0
        # 4. 답안 제출
        if (
            session.post(TEST_STOP_URL, json={"course_id": 45}, timeout=5).status_code
            != 200
        ):
            return False, 0
        # 5. 재응시 초기화
        if (
            session.post(TEST_RESET_URL, json={"course_id": 45}, timeout=5).status_code
            != 200
        ):
            return False, 0
...
def test_load_profile_with_pytest(target_users):
    ...
    total_requests = target_users * 3  # 유저당 3회 루프 검증
    success_count = 0
    fail_count = 0
    all_latencies = []
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=target_users) as executor:
        for _ in range(3):  # Loop 3회 반복
            futures = [
                executor.submit(execute_single_user_flow, accounts[i])
    ...
    # 💡 1. 에러율 기준 검증 (모든 단계 공통 < 1%)
    assert error_rate < 1.0, (
        f"부하 테스트 실패: 에러율이 1%를 초과했습니다 ({error_rate}%)"
    )

    # 💡 2. 단계별 평균 Latency 기준 동적 검증 (계획서 기준 반영)
    if target_users == 5:
        max_allowed_latency = 1000  # 5명: < 1000ms
    elif target_users == 10:
        max_allowed_latency = 2000  # 10명: < 2000ms
    else:
        max_allowed_latency = 5000  # 20명, 30명: < 5000ms
